...
- `프싴`, `쿼싴`, `시크`, `싴`, `울레`, `울`, `슈레`, `슈`, `얼티`, `얼`처럼 같은 의미의 약칭은 공통 레어도 값으로 정규화한다.
- `오버프싴`, `오버울`처럼 별도 상품군으로 쓰이는 복합 표현은 기본 레어도에 흡수하지 않고 독립된 값으로 보존한다.
- `후와로스 슈레 0.3 컬레 0.5 시크 0.7`처럼 한 줄에 레어도와 가격이 반복되면 레어도마다 관측행을 만들고, 모든 행에 같은 `raw_line`과 게시글 URL을 남긴다.
- `프싴` 같은 제목형 레어도 구간은 다음 구간이 시작될 때까지 가격행에 상속한다. 원문에 레어도가 없는 행을 임의의 카드 레어도로 확정하지 않는다.
- 웹 탐색기에서는 카드 키와 레어도를 함께 묶어 카드·레어도별 공급, 수요, 관측 중앙값을 확인한다.
