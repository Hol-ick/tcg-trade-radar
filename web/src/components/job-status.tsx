    ...
    <Card className="ink-panel border-0 shadow-none">
      <CardHeader className="border-b border-white/10 px-5 py-5 sm:px-6">
        <div className="flex items-center justify-between gap-3">
          <div>
            <div className="eyebrow mb-2 text-orange-300">02 / EVIDENCE STREAM</div>
            <CardTitle className="text-xl text-white">실행 상태</CardTitle>
          </div>
          {state && StateIcon ? (
            <Badge className={state.className}>
              <StateIcon className={`size-3.5 ${job?.state === "running" ? "animate-spin" : ""}`} />
              {state.label}
            </Badge>
          ) : (
            <Badge className="bg-white/10 text-slate-400">아직 실행 전</Badge>
          )}
        </div>
      </CardHeader>
      <CardContent className="space-y-5 px-5 py-5 sm:px-6">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-xs text-slate-500">
            <span>{job ? `job ${job.id.slice(0, 8)}` : "job id가 생성되면 로그가 여기에 흐릅니다"}</span>
            <span>{isPolling ? "worker polling · 1.2s" : job ? state?.label : "standby"}</span>
          </div>
        ...
        <div className="grid gap-2 text-xs text-slate-500 sm:grid-cols-2">
          <p>worker: <span className="text-slate-300">{job?.worker_version || "연결 대기"}</span></p>
          <p className="sm:text-right">finished: <span className="text-slate-300">{formatDate(job?.finished_at)}</span></p>
        </div>
      </CardContent>
    </Card>
  )
...
function Metric({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-xl border border-white/10 bg-white/[0.035] p-3">
      ...
      <div className="mt-1 text-2xl font-semibold tracking-tight text-white">{value.toLocaleString()}</div>
    </div>
  )
}
