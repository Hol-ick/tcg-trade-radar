...
export function ResultTable({ rows, isLoading, onDownloadCsv }: { rows: ResultRow[]; isLoading?: boolean; onDownloadCsv?: () => void }) {
  return <section className="results-panel" aria-labelledby="results-title">
    <div className="results-heading"><h2 id="results-title"><PackageSearch size={18} aria-hidden="true" /> 수집 결과</h2><div className="results-actions"><span className="count-badge">{rows.length}건</span>{onDownloadCsv && <button className="secondary-button" type="button" onClick={onDownloadCsv}><Download size={15} aria-hidden="true" /> CSV 저장</button>}</div></div>
    {isLoading ? <div className="empty-state">결과를 불러오는 중입니다.</div> : rows.length === 0 ? <div className="empty-state">아직 수집된 결과가 없습니다.</div> : <div className="table-wrap"><table><thead><tr><th>카드 / 게시글</th><th>가격</th><th>수량</th><th>분류</th><th>판매자 / 위험도</th><th>검토</th><th>원문</th></tr></thead><tbody>{rows.map((row, index) => <ResultRow key={row.id || `${row.post_url}-${index}`} row={row} />)}</tbody></table></div>}
  </section>
}
...
function ResultRow({ row }: { row: ResultRow }) {
  ...
  return <tr><td><strong>{title}</strong><span>{row.post_title || row.raw_line || "제목 없음"}</span></td><td className="price">{formatWon(row.price_krw)}<small className="quality-note">{priceLabel(row.price_status, row.price_scope)}</small></td><td>{row.quantity || 1}</td><td><span className="type-chip">{typeLabel(row.listing_type)}</span><small className="quality-note">{postLabel(row.post_status)}</small></td><td><strong className="seller-name">{row.seller_name || row.author_name || "미상"}</strong><small className={`risk-chip risk-${row.seller_risk_level || "low"}`}>{riskLabel(row.seller_risk_level, row.seller_risk_score, row.is_repost)}</small></td><td><span className={`review-chip ${row.analysis_status === "usable" && row.review_status !== "needs_review" ? "ok" : "review"}`}>{analysisLabel(row.analysis_status, row.review_status)}</span></td><td>{url && <a className="row-link" href={url} target="_blank" rel="noreferrer" aria-label={`${title} 원문 열기`}><ExternalLink size={15} /></a>}</td></tr>
}
...
function priceLabel(status?: string, scope?: string): string { if (status === "missing") return "가격 없음"; if (status === "removed") return "가격 삭제"; if (scope === "bundle") return "묶음가격"; if (scope === "per_quantity") return "복수 수량가"; return status === "estimated" ? "단위 추정" : "" }
...
function riskLabel(level?: string, score?: number, repost?: number): string { const suffix = repost ? " · 반복" : ""; return `${level === "high" ? "높은" : level === "medium" ? "주의" : "낮은"} 위험 ${score || 0}${suffix}` }
