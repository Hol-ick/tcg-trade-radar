  ...
  return <main className="explorer-shell collector-shell">
    <aside className="saas-sidebar">
      ...
      <nav className="sidebar-nav" aria-label="주 메뉴">
        <a href={import.meta.env.BASE_URL}><Database size={16} />Overview</a>
        <a href={`${import.meta.env.BASE_URL}#signals`}><Database size={16} />Card signals</a>
        <a className="active" href="?page=collector"><MonitorDown size={16} />Collector</a>
      </nav>
      ...
      <div className="sidebar-source">
        ...
        <span><strong>GitHub CSV catalog</strong><small>54 analysis partitions</small></span>
      </div>
    ...
    <section className="saas-main">
      <header className="saas-header">
        <div className="breadcrumbs"><span>Workspace</span><span>/</span><strong>Collector</strong></div>
        <div className="header-actions"><span className="connection-pill"><i />Desktop app guide</span><a className="header-collector-link" href={import.meta.env.BASE_URL}>Back to explorer <ArrowUpRight size={14} /></a></div>
      </header>

      <div className="saas-content collector-content">
        <section className="page-heading collector-heading">
          <div><span className="eyebrow">COLLECTOR</span><h1>수집은 데스크톱 앱에서 실행합니다.</h1><p>GitHub Pages는 브라우저 보안상 사용자의 PC에서 EXE를 직접 실행할 수 없습니다. 로컬 수집기를 실행한 뒤 결과 CSV를 Git에 올리면, 이 페이지가 최신 카탈로그를 읽어 분석합니다.</p></div>
          <a className="button button-primary" href={import.meta.env.BASE_URL}><ArrowLeft size={16} />Market explorer</a>
        </section>
        ...
        <div className="collector-status-grid">
          <article className="collector-status-card">
            ...
            <div><span className="eyebrow">LOCAL APP</span><h2>데스크톱 수집기</h2><p>게임·기간·게시글 수를 설정하고 원문, 댓글, 정규화된 관측행을 로컬 SQLite에 저장합니다.</p><a className="inline-link" href={RUNNER_URL} target="_blank" rel="noreferrer">실행 파일 보기 <ExternalLink size={13} /></a></div>
          </article>
          <article className="collector-status-card">
            <span className="collector-card-icon green"><CheckCircle2 size={18} /></span>
            <div><span className="eyebrow">PUBLISHED DATA</span><h2>GitHub CSV 카탈로그</h2><p>게임·월·판매/구매/교환별 파티션을 선택하면 해당 CSV만 브라우저로 가져옵니다.</p><a className="inline-link" href={CATALOG_URL} target="_blank" rel="noreferrer">카탈로그 열기 <ExternalLink size={13} /></a></div>
          </article>
        </div>

        <section className="collector-flow-card">
          <div className="section-heading"><div><span className="eyebrow">WORKFLOW</span><h2>수집에서 분석까지</h2></div><span className="section-meta">한 번 수집하고, 필요한 파티션만 조회</span></div>
          <div className="collector-steps">
            ...
            <CollectorStep number="04" icon={<FileDown size={17} />} title="Explorer에서 선택" description="Market explorer의 데이터 소스 선택기에서 GitHub CSV 파티션을 골라 가격·수요·공급을 분석합니다." />
          </div>
        </section>

        <section className="collector-help-grid">
          <article className="collector-help-card"><span className="eyebrow">QUICK START</span><h2>Windows에서 시작</h2><pre><code>cd D:\\Files\\TASKS\\PROJECTS\\tcg_trade_radar
